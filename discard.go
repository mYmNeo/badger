...
func InitDiscardStats(opt Options) (*discardStats, error) {
	...
	// 1MB file can store 65536 discard entries. Each entry is 16 bytes.
	mf, err := y.OpenMmapFile(fname, os.O_CREATE|os.O_RDWR, 1<<20)
	lf := &discardStats{
		MmapFile: mf,
		opt:      opt,
	}
...
func (lf *discardStats) HasEntry(fidu uint32) bool {
	fid := uint64(fidu)
	lf.Lock()
	defer lf.Unlock()
	...
	return idx < lf.nextEmptySlot && lf.get(idx*16) == fid
}
